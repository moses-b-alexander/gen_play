...
def parse_snap_x_range(raw: str) -> tuple[float, float]:
    lo, hi = raw.split(",")
...
def build_run_config(cfg: dict[str, str]) -> dict:
    postprocess_kwargs = dict(
        ...
        sxr=parse_snap_x_range(cfg["snap_x_range"]),
    ...
    diff_eq_kwargs = dict(
        ...
        steps=int(cfg["sde_steps"]),
        rtol=1e-5, atol=1e-5, dt=1e-2,
    )
    decoder_kwargs = dict(
